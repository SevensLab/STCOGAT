...
def _morans_I_scores(X, W, center=True):
    ...
    I_g = (n/Wsum) * (v^T W v)/(v^T v), v=x-mean(x)
    """
    n = W.shape[0]
    Xw = X.copy()
    if center:
    ...
    Wsum = float(W.sum())
    WV = W @ Xw
    num = (Xw * WV).sum(axis=0)        # ∑_i ∑_j w_ij v_i v_j
    den = (Xw ** 2).sum(axis=0) + 1e-12
...
def low_pass_enhancement(
    ...
    if not weighted:
        A = kneighbors_graph(coords, n_neighbors=num_neighbors, mode='connectivity', include_self=False).tocsr()
        A = A.maximum(A.T).astype(float)
        A.eliminate_zeros()
    ...
    vals, vecs = spla.eigsh(L.astype(float), k=k_low_val, which='SM')  # vals: [k], vecs: [n,k]

    h = (1.0 / (1.0 + c * np.maximum(vals, 0.0))) ** float(power)   # shape [k]
...
def svg(adata, svg_method='gft_top', n_top=3000, csvg=0.0001, smoothing=True):
    assert svg_method in ['gft', 'gft_top', 'seurat', 'seurat_v3','cell_ranger']
    if svg_method == 'seurat_v3':
        ...
        if svg_method == 'seurat':
            ...
            sel_genes = select_svg(adata,N=n_top,
                                    spatial_key='spatial',
                                    weighted=True,
                                    normalize_lap=True,
                                    mutual=True,
            ...
            adata_raw = adata.copy()
            if smoothing:
                low_pass_enhancement(adata,ratio_low_freq=15,
                                     c=csvg,
