...
class TripleviewEncoder(torch.nn.Module):
    def __init__(self, col_dim, row_dim, num_layers=1, drop_p=0.1,
                 use_rows=True, row_latent_dim=256):
        super().__init__()
        self.col_dim = col_dim        
        self.row_dim = row_dim          
        self.num_layers = num_layers
        self.use_rows = use_rows
        self.row_latent_dim = row_latent_dim
        ...
        if self.use_rows:
            self.row_down = nn.Linear(self.row_dim, self.row_latent_dim, bias=False)
            self.row_up   = nn.Linear(self.row_latent_dim, self.row_dim, bias=False)

            self.rows_layers = nn.ModuleList([
                sequential.Sequential(
                    "x,edge_index",
                    [
                        (SAGEConv(self.row_latent_dim, self.row_latent_dim), "x, edge_index -> x1"),
                        (nn.Dropout(drop_p, inplace=False), "x1 -> x2"),
                        nn.LeakyReLU(inplace=True),
                    ],
                )
    ...
    def forward(self, x, knn_edge_index, genet_edge_index):
        ...
        for i in range(self.num_layers):
            emb = self.cols_layers[i](emb.T, knn_edge_index).T
            if self.use_rows:
                h = self.row_down(emb)                      
                h = self.rows_layers[i](h, genet_edge_index) 
                emb = self.row_up(h)                        
        return emb
...
class GCNEncoder(torch.nn.Module):
    def __init__(
        self, feature_dim, inter_dim, embd_dim, reducer=False, drop_p=0.1, scale_param=3
    ):
    ...
    def reduce_network(self, threshold=0.1, min_connect=4):
        self.plot_count += 1
        graph = self.atten_weights.detach().cpu().numpy().reshape(-1)

        threshold_bound = np.percentile(graph, 10)
        threshold = min(threshold, threshold_bound)

        v1 = self.atten_map[0].detach().cpu().numpy()
        v2 = self.atten_map[1].detach().cpu().numpy()
        df = pd.DataFrame({"v1": v1, "v2": v2, "atten": graph})
        keep_mask = df["atten"].values >= threshold
        topk_idx = (
            df.groupby("v1")["atten"]
            .nlargest(min_connect)
            .index
            .get_level_values(1)   
            .to_numpy()
        )
        ...
        indexs = np.nonzero(keep_mask)[0].tolist()
...
class STCOGAT(torch.nn.Module):
    def __init__(
        ...
        self.encoder = TripleviewEncoder(col_dim, row_dim, num_layers, drop_p, use_rows=use_rows_encoder)
        if self.use_rows_encoder:
            self.rows_encoder = GCNEncoder(
                row_dim,inter_row_dim,embd_row_dim,reducer=False,drop_p=drop_p,scale_param=None,
            )
        self.cols_encoder = GCNEncoder(
        ...
        self.feature_decoder = FeatureDecoder(col_dim, embd_col_dim + pre_emb_dim, inter_col_dim, drop_p=0.0)
    ...
    def recon_loss_edgewise(self,z,pos_edge_index,     # [2, E]
        neg_edge_index=None,num_neg: int = 1,temperature: float = 0.2,):
        if not torch.is_tensor(pos_edge_index):
            pos_edge_index = torch.tensor(pos_edge_index)
        pos_edge_index = pos_edge_index.to(z.device).long()

        if neg_edge_index is None:
            num_neg_samples = max(1, int(pos_edge_index.size(1) * num_neg))
            ...
            if not torch.is_tensor(neg_edge_index):
        ...
        pos_sim = (zi * zj).sum(dim=1) / temperature

        ni = z[neg_edge_index[0]]
        nj = z[neg_edge_index[1]]
        neg_sim = (ni * nj).sum(dim=1) / temperature

        pos_loss = -torch.log(torch.sigmoid(pos_sim) + EPS).mean()
        neg_loss = -torch.log(1.0 - torch.sigmoid(neg_sim) + EPS).mean()
        return pos_loss + neg_loss
    ...
    def calculate_loss(
        ...
        knn_pre_emb = self.knn_pre_mlp(x.T)
        ...
        E = genet_edge_index.size(1)
        if E == 0:
            reg = out_features.new_zeros(())  
        else:
            reg = self.recon_loss_edgewise(out_features.T, genet_edge_index, num_neg=5, temperature=0.2)
        ...
        diff = (z[row] - z[col]).pow(2).mean(dim=1)  
        ...
        if self.use_rows_encoder:
            return loss, col_loss,row_loss,contrastive_loss,loss_smooth,reg
        return loss, col_loss,contrastive_loss,loss_smooth, reg
